...
def train(rank, a, h):
    ...
    if h.optimizer:
        # This code was ripped from ZFTurbo's Music Source Separation Training repo
        optim_params = dict() # I don't think this is ever used so I'll simplify it

        if h.optimizer == 'adam':
            optim_g = Adam(generator.parameters(), h.learning_rate, betas=[h.adam_b1, h.adam_b2])
            optim_d = Adam(discriminator.parameters(), h.learning_rate, betas=[h.adam_b1, h.adam_b2])
        elif h.optimizer == 'adamw':
            optim_g = AdamW(generator.parameters(), h.learning_rate, betas=[h.adam_b1, h.adam_b2])
            optim_d = AdamW(discriminator.parameters(), h.learning_rate, betas=[h.adam_b1, h.adam_b2])
        elif h.optimizer == 'radam':
            optim_g = RAdam(generator.parameters(), lr=h.learning_rate, betas=[h.adam_b1, h.adam_b2])
            optim_d = RAdam(discriminator.parameters(), lr=h.learning_rate, betas=[h.adam_b1, h.adam_b2])
        elif h.optimizer == 'rmsprop':
            optim_g = RMSprop(generator.parameters(), lr=h.learning_rate, **optim_params)
            optim_d = RMSprop(discriminator.parameters(), lr=h.learning_rate, **optim_params)
        elif h.optimizer == 'prodigy':
            from prodigyopt import Prodigy
            # you can choose weight decay value based on your problem, 0 by default
            # We recommend using lr=1.0 (default) for all networks.
            optim_g = Prodigy(generator.parameters(), lr=h.learning_rate, **optim_params)
            optim_d = Prodigy(discriminator.parameters(), lr=h.learning_rate, **optim_params)
        elif h.optimizer == 'adamw8bit':
            import bitsandbytes as bnb
            optim_g = bnb.optim.AdamW8bit(generator.parameters(), lr=h.learning_rate, **optim_params)
            optim_d = bnb.optim.AdamW8bit(discriminator.parameters(), lr=h.learning_rate, **optim_params)
        elif h.optimizer == 'sgd':
            print('Use SGD optimizer')
            optim_g = SGD(generator.parameters(), lr=h.learning_rate, **optim_params)
            optim_d = SGD(discriminator.parameters(), lr=h.learning_rate, **optim_params)
        else:
            print('Unknown optimizer: {}'.format(config.training.optimizer))
            if h.adam_b1 and h.adam_b2:
                print("AdamW settings found, defaulting to adamw")
                optim_g = AdamW(generator.parameters(), h.learning_rate, betas=[h.adam_b1, h.adam_b2])
                optim_d = AdamW(discriminator.parameters(), h.learning_rate, betas=[h.adam_b1, h.adam_b2])
            else:
                exit()
